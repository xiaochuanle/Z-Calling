...
{
    hbn_assert(!m_UngetLine && m_Line.begin());
...
x_load_data(gzFile stream, char* buffer, int count, int* bytes_read)
{
    ...
    if (n < count) {
        if (gzeof(stream)) {
            ...
            int the_errno;
            const char* why = gzerror(stream, &the_errno);
            if (the_errno) HBN_ERR("%s", why);
        }
    }
    *bytes_read = n;
    return result;
...
CTempString CBufferLineReader::operator*() const 
{
