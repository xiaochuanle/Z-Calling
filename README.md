...
2. **Signal Quality:** Reads with Median_PW / Median_IPD < 0.3 or > 99th percentile are discarded.  
...
1. **model/k21-full-ZA/scripted_m21.pth** (Use with -k 21)  
...
2. **model/k11-mixed-AZ/scripted_m11.pth** (Use with -k 11)  
...
```bash
build/z-calling-base -k <Kmer_size> -t <Threads> <Input_BAM> <Model_Path> <Output_BAM>
...
| Model_Path | Path to the .pth model file (see list above). |
| Output_BAM | Output BAM file. Modifications are stored in MM and ML tags. |
```
...
```bash
build/z-calling-read predict <Input_BAM> <Result_TSV> <Model_Path> <min_len> <intervals>
```
* **Model_Path**: Path to the SVM model (e.g., model/svm/k21ReadClassifier).  
* **min_len**: Minimum read length to classify (Required: must be **>= 500**).  
